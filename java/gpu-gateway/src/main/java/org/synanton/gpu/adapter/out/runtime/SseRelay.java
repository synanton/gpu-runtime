...
final class SseRelay {
    ...
    /** {@code finalResult}: the completed Responses API object (RESPOND streams only). */
    record Outcome(ExecutionUsage usage, boolean sawDone, boolean timedOut, byte[] finalResult) {
        Outcome(ExecutionUsage usage, boolean sawDone, boolean timedOut) {
            this(usage, sawDone, timedOut, null);
        }
    }

    /** Responses API terminal event types (Plan §10.4): the stream ends with one of these. */
    static final java.util.Set<String> RESPONSES_TERMINAL_TYPES =
            java.util.Set.of("response.completed", "response.incomplete", "response.failed");

    /** Usage from either chat ({@code prompt/completion_tokens}) or Responses ({@code input/output_tokens}). */
    static ExecutionUsage usageOf(JsonNode u, long startMs, String runtimeClass) {
        return new ExecutionUsage(
                u.path("prompt_tokens").asLong(u.path("input_tokens").asLong(0)),
                u.path("completion_tokens").asLong(u.path("output_tokens").asLong(0)),
                (System.currentTimeMillis() - startMs) / 1000.0, runtimeClass);
    }

    /**
     * Responses API stream relay (RESPOND): typed events forwarded as {@code data:} frames
     * (the event's JSON carries its {@code type}); nested {@code response.model} restored to
     * the logical ID; ends at the first terminal event — {@code [DONE]} is never forwarded
     * (§10.4). The terminal event's response object is returned as {@code finalResult}.
     */
    static Outcome relayResponses(Stream<String> lines, String logicalModelId, ObjectMapper json,
                                  StreamChunkSink sink, String runtimeClass, long startMs,
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                if (!line.startsWith("data:")) {
                    continue; // "event:" lines: the type is inside the JSON
                }
                String data = line.substring(5).trim();
                if ("[DONE]".equals(data)) {
                    continue; // never the Responses terminal protocol (§10.4)
                }
                JsonNode event;
                try {
                    event = json.readTree(data);
                } catch (Exception e) {
                    log.warn("runtime {} sent a non-JSON Responses event; dropped", runtimeClass);
                    continue;
                }
                ...
                JsonNode response = object.get("response");
                if (response instanceof ObjectNode r && r.has("model")) {
                    r.put("model", logicalModelId);
                }
                try {
                    sink.onChunk(("data: " + json.writeValueAsString(object) + "\n\n").getBytes(StandardCharsets.UTF_8));
                } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
                    throw new IllegalStateException("cannot re-serialize Responses event", e);
                }
                if (RESPONSES_TERMINAL_TYPES.contains(object.path("type").asText())) {
                    ExecutionUsage usage = response != null && response.path("usage").isObject()
                            ? usageOf(response.get("usage"), startMs, runtimeClass) : null;
                    byte[] result;
                    try {
                        result = response != null ? json.writeValueAsBytes(response) : null;
                    } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
                        throw new IllegalStateException(e);
                    }
                    return new Outcome(usage, true, false, result);
                }
            }
            return new Outcome(null, false, timedOut.get());
    ...
    private static Outcome relayUntilDone(Stream<String> lines, String logicalModelId, boolean forwardUsage,
        ...
        while (!sawDone && it.hasNext()) {
            ...
            if (usageChunk) {
                usage = usageOf(object.get("usage"), startMs, runtimeClass);
                if (!forwardUsage) {
